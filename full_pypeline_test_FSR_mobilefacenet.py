...
try:
    ...
    print("   Super Resolution lipseste\n")
...
if HAS_CV2_CONTRIB:
    try:
        if os.path.exists(SR_MODEL_PATH):
            ...
            use_sr = True
            print(f"SRCNN Activat (Scale x{SR_SCALE})!")
        else:
...
def get_embedding(full_image_bgr, coords, model):
    ...
    if x2 - x1 < 10 or y2 - y1 < 10: return None, False
...
print(f"\n START EVALUARE PE {len(dataset)} IMAGINI (FARA FLIP TRICK)")
print("-" * 80)
...
for i in range(len(dataset)):
    ...
    if len(detections) == 0:
        print(f"{filename:<35} | {real_name:<10} | {'---':<15} | {'0.00'} | ⚠️ | -")
    ...
    for det in detections:
        ...
        res_emb, upscaled_flag = get_embedding(frame, box, embed_model)
    ...
    if VISUALIZE:
        ...
        cv2.imshow("Test MobileFaceNet + FSRCNN (No Flip)", frame)
